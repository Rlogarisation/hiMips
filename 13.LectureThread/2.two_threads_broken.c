...
int main(void) {
    ...
    // Only one initiation for two threads.
    int thread_number = 1;
    pthread_create(&thread_id1, NULL, run_thread, &thread_number);
    // Then change the value to 2 for second thread.
    // However because the property of threads are running cocurrenctly,
    // complier will read thread_number = 2 first before pthread_create runs.
    thread_number = 2;
