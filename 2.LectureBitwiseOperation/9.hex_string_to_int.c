...
uint32_t hex_string_to_int(char *hex_string) {
    uint32_t value = 0;
    // Why the program will finish if hex_string[which_digit] != 0 ?
    // Because this is in terms of ASCII table, 0 is NULL, 
    // hence 0 can be replaced by NULL in here.
...
int hex_digit_to_int(int ascii_digit) {
    ...
    if (ascii_digit >= 'A' && ascii_digit <= 'F') {
        ...
        // Why there is a extra 10 has been added for the return?
        // Answer: So it has to match the pattern of "0123456789ABCDEF"
