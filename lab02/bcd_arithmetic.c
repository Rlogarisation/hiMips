...
bcd_add, bcd_subtract, bcd_multiply, and bcd_divide so that, 
given two arbitrary-length binary-coded decimal (BCD) numbers, 
they return the result of the corresponding arithmetic operation. 
For example:
...
and calls bcd_add, bcd_subtract, bcd_multiply, and bcd_divide respectively. 
...
Use realloc(3) to grow an array.

You could use Python (for example) to check what the value of an expression is, 
(though you don't need to quote the *, 
and you need to use // to get integer division):
